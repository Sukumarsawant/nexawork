        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2 flex flex-col gap-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="bg-[#3E3E55]/50 rounded-2xl p-6">
                <h2 className="font-semibold mb-3">Education & Certifications</h2>
                <ul className="text-sm text-gray-300 space-y-2">
                  {profile?.education && Object.values(profile.education).map((edu, i) => (
                    <li key={i}>{edu}</li>
                  ))}
                  {profile?.certifications && Object.values(profile.certifications).map((cert, i) => (
                    <li key={i}>{cert}</li>
                  ))}
                  {(!profile?.education && !profile?.certifications) && (
                    <p className="text-gray-500 text-sm">No data added</p>
                  )}
                </ul>
              </div>
